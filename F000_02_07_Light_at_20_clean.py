...
def kkI(What_is_limiting,m,Pmax,OT,Ynphoto_chl,Cnbiosynth,Nconst_protein,Nstore_max,Cnrna_variable,Ypthylakoid_chl,Pconst_other,Qp_max,Cessential,CNprotein):   #this function calculate for the same irradiance
    ...
    #--------------------------
    QcProchloro = (49.2e-15)/12 #(mol cell-1) (See the paper draft "05 K table s1.docx")
    ...
    #CNprotein = 3.82 #(molC molN) from Geider 2002
    #-------------------
    #C protein
    #-------------------
    ...
    Mchl=892.5012             #(g / mol chlorophyll) mollar mass of chlorophyll
   ...
   # sup(1)
    ...
    title('PCC 9515, Light: '+str(I)+' $\mu$mol m$^{-2}$ s$^{-1}$', y=1.05)
    xlim(0,0.4)
    # sup(2)
...
# Parameter sets
#=============================
CNprotein = 5.3     #(molC molN) picked this value to fit N:C after fitting macromolecular
...
Pmax=0.003355   # See "Fig3.xlsx"
OT= 0.017143    # See "Fig3.xlsx"

Ynphoto_chl= 3.56099164557551*pf          #((molN cell-1)/(molC chl cell-1)) the stoichiometric ratio for cell photosynthetic enzyme (Rubisco etc.) nitrogen to chlorophyll (193-25)
Cnbiosynth= 4.34728279914354E-10*pf        #(molN cell-1 s) Constant for varible part of biosynthesis protein nitrogen (193-37)
Nconst_protein= 6E-15*pf     #(molN cell-1) Constant protein pool in nitrogen (193-25)
...
Cessential= 2.7E-14          #(molC cell-1) essential carbon (lipid membrane, etc.) *8.33e-14/10 is 10%
